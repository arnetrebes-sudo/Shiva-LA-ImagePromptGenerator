...
const App: React.FC = () => {
  ...
  useEffect(() => {
    let interval: number;
    if (visualizingIds.size > 0 || editingPromptId) {
      interval = window.setInterval(() => {
        setLoadingStepIndex(prev => (prev + 1) % VISUALIZING_STEPS.length);
      }, 2000);
    }
    return () => clearInterval(interval);
  }, [visualizingIds.size, editingPromptId]);
      ...
      <header className="bg-white/70 dark:bg-slate-900/70 backdrop-blur-md border-b border-orange-100 dark:border-slate-800 py-6 px-4 sticky top-0 z-50">
        <div className="max-w-6xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-4">
            <div className="w-12 h-12 rounded-2xl border border-orange-100 dark:border-slate-700 overflow-hidden shadow-sm bg-orange-50 dark:bg-slate-800 flex-shrink-0 flex items-center justify-c[...]
            <div>
          ...
          <div className="flex items-center gap-3">
             <button onClick={() => setActiveTab('saved')} className={`hidden md:flex items-center gap-2 text-sm font-semibold transition-colors ${activeTab === 'saved' ? 'text-orange-500' : 'tex[...]
      ...
      <section className="bg-orange-500 dark:bg-orange-600 py-4 px-4 shadow-md z-40">
        <div className="max-w-6xl mx-auto flex items-center justify-center gap-3 text-white text-center">
          ...
          <p className="text-sm font-bold tracking-tight leading-snug">
            This is a <span className="underline decoration-white/50 underline-offset-4 font-black">LA Shiva Tool</span> designed to help Landscapearchitects and Planners with fast sketching of n[...]
          </p>
        </div>
      </section>
            ...
            <div className="flex justify-between items-start mb-6">
              ...
              <div className="flex items-center gap-3">
                <button 
                  ...
                  className="flex items-center gap-2 px-4 py-2 rounded-xl bg-orange-50 dark:bg-orange-900/20 text-orange-600 dark:text-orange-400 text-xs font-bold hover:bg-orange-100 transition-[...]
                >
                ...
                {pendingUploads.map((item, index) => (
                  <div key={index} className="flex flex-col p-4 rounded-3xl border border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-950/30">
                    <div className="relative aspect-video rounded-2xl overflow-hidden border border-slate-200 dark:border-slate-700 mb-4">
                      ...
                      <button onClick={() => removePendingUpload(index)} className="absolute top-2 right-2 p-1.5 bg-red-500/90 text-white rounded-lg shadow-lg hover:scale-110 active:scale-95 tran[...
                    ...
                    <div className="space-y-3">
                      <input 
                        ...
                        className="w-full p-2.5 rounded-xl border border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-950 text-xs font-bold outline-none focus:ring-2 ring-orange-100 tra[...
                      />
                      <textarea 
                        placeholder="Technical description..."
                        value={item.prompt} 
                        onChange={(e) => updatePendingUpload(index, 'prompt', e.target.value)} 
                        className="w-full p-2.5 h-16 rounded-xl border border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-950 text-[11px] outline-none focus:ring-2 ring-orange-100 tran[...
                      />
                    </div>
                  </div>
                ))}
      ...
      <main className="flex-1 max-w-6xl mx-auto w-full p-4 md:p-10 grid grid-cols-1 lg:grid-cols-12 gap-10">
        <section className="lg:col-span-5 space-y-8">
          <div className="bg-white dark:bg-slate-900 rounded-[2rem] shadow-sm border border-orange-50 dark:border-slate-800 p-8">
            ...
            <div className={`overflow-hidden transition-all duration-500 ease-in-out ${showTemplates ? 'max-h-[500px] mb-6 opacity-100' : 'max-h-0 opacity-0'}`}> 
              <div className="flex flex-wrap gap-2 p-1 border-t border-slate-100 dark:border-slate-800 pt-4">
                <button 
                  ...
                  className="flex items-center gap-1.5 px-3 py-1.5 rounded-full text-[10px] font-bold transition-all border bg-slate-900 text-white border-slate-900 hover:bg-black dark:bg-slate-1[...]
                >
                ...
                {PRESET_TEMPLATES.map(tpl => (
                  <button 
                    ...
                    className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-[10px] font-bold transition-all border ${activeTemplateId === tpl.id ? 'bg-orange-400 border-orange-400 tex[...] 
                  >
            ...
            <textarea 
              className="w-full h-48 p-5 rounded-2xl border border-slate-100 dark:border-slate-800 bg-slate-50/30 dark:bg-slate-950/30 focus:bg-white dark:focus:bg-slate-950 focus:ring-4 focus:ri[...] 
            ...
            <div className="mb-6">
              {!referenceImage ? (
                <button onClick={() => fileInputRef.current?.click()} className="w-full border-2 border-dashed border-slate-100 dark:border-slate-800 rounded-2xl p-6 hover:border-orange-200 hover[...] 
                ...
                <div className="relative rounded-2xl overflow-hidden border border-orange-100 dark:border-slate-800"><img src={`data:${referenceImage.mimeType};base64,${referenceImage.data}`} cla[...] 
              )}
            </div>
            ...
            <button onClick={handleGenerate} disabled={isGenerating || !concept.trim()} className="w-full bg-orange-400 hover:bg-orange-500 disabled:bg-slate-200 text-white font-bold py-4.5 round[...] 
            <button onClick={handleRenderAll} disabled={isBulkVisualizing || prompts.length === 0} className="w-full mt-4 bg-slate-900 dark:bg-slate-100 text-white dark:text-slate-900 font-bold p[...] 
          </div>
        </section>
        ...
        <section className="lg:col-span-7 space-y-8">
          <div className="flex items-center justify-between px-2">
            <div className="flex gap-4">
              <button onClick={() => setActiveTab('recent')} className={`flex items-center gap-2 text-sm font-bold transition-all px-4 py-2 rounded-full ${activeTab === 'recent' ? 'bg-orange-100 [...]'}><History className="w-4 h-4" /> Recent</button>
              <button onClick={() => setActiveTab('saved')} className={`flex items-center gap-2 text-sm font-bold transition-all px-4 py-2 rounded-full ${activeTab === 'saved' ? 'bg-orange-100 te[...] )}<Bookmark className="w-4 h-4" /> Saved</button>
            </div>
          </div>
          ...
          <div className="space-y-8">
            ...
            {displayedPrompts.map((prompt) => (
              <div key={prompt.id} className="bg-white dark:bg-slate-900 rounded-[2rem] shadow-sm border border-slate-100 dark:border-slate-800 overflow-hidden transition-all duration-300">
                <div className="p-8">
                  ...
                  {editingPromptId === prompt.id ? (
                    <div className="mb-6 animate-in fade-in slide-in-from-top-2 duration-200">
                      <textarea className="w-full p-4 rounded-xl border border-orange-200 dark:border-orange-900/40 bg-orange-50/20 text-[13px] text-slate-600 dark:text-slate-300 focus:ring-2 foc[...] 
                ...
                {(visualizingIds.has(prompt.id) || editingPromptId === prompt.id) && (
                ...
                {visualizedImages[prompt.id] && !visualizingIds.has(prompt.id) && !visualizationErrors[prompt.id] && editingPromptId !== prompt.id && (
        ...
        <section className="lg:col-span-12 mt-12 pb-12 border-t border-orange-100 dark:border-slate-800 pt-16">
          <div className="flex flex-col items-center mb-10"><h2 className="text-3xl font-serif font-bold text-orange-900 dark:text-orange-100 mb-2">Showcase Archive</h2><div className="h-1 w-24 b[...] 
          ...
          <div className="flex flex-col items-center">
            <div className="bg-white dark:bg-slate-900 border border-orange-100 dark:border-slate-800 rounded-3xl p-8 max-w-2xl w-full flex flex-col items-center gap-6 shadow-sm">
              ...
              <div className="flex flex-wrap justify-center gap-4 w-full">
                <button onClick={() => galleryManualRef.current?.click()} className="flex-1 min-w-[200px] bg-slate-900 dark:bg-slate-100 text-white dark:text-slate-900 px-8 py-3.5 rounded-2xl tex[...] 
                  <ImageIcon className="w-4 h-4" /> Select Batch Renders
                </button>
                {galleryImages.length > 0 && (
                  <button onClick={handleClearArchive} className="flex-1 min-w-[200px] border border-red-100 dark:border-red-900/30 bg-red-50 dark:bg-red-900/10 text-red-500 px-8 py-3.5 rounded-2[...]
      ...
      <footer className="bg-white dark:bg-slate-900 border-t border-orange-50 py-12 px-6 transition-colors duration-300">
        <div className="max-w-6xl mx-auto flex flex-col md:flex-row items-center justify-between gap-6">
          <div className="text-center md:text-left">
            <p className="text-slate-400 text-xs font-medium mb-1">© 2024 LA Visual Prompt Engine Version 2.1</p>
